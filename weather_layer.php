	...
	if (isset($args['city']) && isset($args['country']))
	{
		...
		if ($weatherData)
		{
			...
			if ($weatherJson->cod == 200)
			{
				...
				for ($i = ceil(OPENWEATHERMAP_NB_HOURS_PER_DAY / 2); $i < min(NB_DAYS + 1, count($weatherJson->list) / OPENWEATHERMAP_NB_HOURS_PER_DAY) * OPENWEATHERMAP_NB_HOURS_PER_DAY; $i += OPENWEATHERMAP_NB_HOURS_PER_DAY)
